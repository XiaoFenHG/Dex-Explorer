...
newButton("复制该事件的远程(触发)", function()
	return "点按以复制路径"
end, function()
	if selected then
...
newButton("执行事件！", function()
	...
	if succeeded then
		...
		TextLabel.Text = "在执行的过程中遇到了错误.."
	end
end)

--- Gets the calling script (not super reliable but w/e)
newButton("获取脚本", function()
	return "单击以将调用脚本复制到剪贴板\ n警告:nil==无法找到"
end, function()
	if selected then
...
newButton("函数信息", function()
	...
	if selected then
		...
		TextLabel.Text = "搞定了序列化程序生成的函数信息."
	end
end)

--- Clears the Remote logs
newButton("清除当前已有的日志", function()
	return "单击以清除日志"
end, function()
	TextLabel.Text = "清除中..."
	logs = {}
...
newButton("排除(一)", function()
	return "单击以排除此.\n排除遥控器会使Spy忽略它，但它将继续可用."
end, function()
	if selected then
...
newButton("排除(2)", function()
	return "单击以排除所有具有此名称的远程.\n排除会使Spy忽略它，但它将继续可用."
end, function()
	if selected then
...
newButton("第一", function()
	return "按一下以停止这个使用.\n阻止远程不会将其从Spy日志中删除，但不会继续触发服务器."
end, function()
	if selected then
		if selected.Remote.remote then
			...
			TextLabel.Text = "错误！实例可能不再存在，请尝试使用Block (2)."
		end
	end
end)

--- Prevents all remotes from firing that share the same name as the selected.Log remote from the RemoteSpy (still logged)
newButton("Block 2", function()
	return "按一下以停止使用这个.\nBlocking remote不会将其从Spy日志中删除，但不会继续触发服务器."
end, function()
	if selected then
...
newButton("得出源码", function()
	return "试图反编译源脚本\n警告:不超级可靠，nil ==could not find"
end, function()
	if selected then
...
newButton("禁用信息", function()
	return string.format(
		"[%s] 切换功能信息(因为它会导致一些游戏延迟)",
		funcEnabled and "使能够" or "有缺陷的"
	)
end, function()
...
newButton("自动拦截", function()
	return string.format(
		"[%s] [测试版本] 智能检测并从日志中排除垃圾远程呼叫",
		autoblock and "使能够" or "有缺陷的"
	)
end, function()
	autoblock = not autoblock
	TextLabel.Text = string.format(
		"[%s] [BETA]智能检测并从日志中排除垃圾远程呼叫",
...
newButton("呼叫脚本", function()
	return string.format(
		"[%s] [UNSAFE]使用“getcallingscript”获取用于反编译和GetScript的调用脚本。更加可靠，但是也给Spy带来了检测和/或不稳定性.",
		useGetCallingScript and "使能够" or "有缺陷的"
	)
end, function()
...
newButton("键盘字符串", function()
	return string.format(
		"[%s] [BETA]使用一个实验性的新函数来复制当一个非原始类型被用作表中的一个键时Roblox的行为。仍在开发中，可能无法正确反映字符串(空)用户数据.",
		keyToString and "使能够" or "有缺陷的"
	)
end, function()
	keyToString = not keyToString
	TextLabel.Text = string.format(
		"[%s] [BETA]使用一个实验性的新函数来复制当一个非原始类型被用作表中的一个键时Roblox的行为。仍在开发中，可能无法正确反映字符串(空)用户数据.",
		keyToString and "正确的" or "缺陷的"
	)
end)

newButton("切换返回值", function()
	return string.format(
		"[%s] [EXPERIMENTAL]允许记录“GetReturnValue”的返回值'\n\n使用这种方法要自担风险，因为它可能会被发现.",
		recordReturnValues and "正确的" or "缺陷的"
	)
end, function()
...
newButton("获取返回值", function()
	return "[Experimental]如果启用了“ReturnValues ”,将显示记录的RemoteFunction的返回值(如果可用)."
end, function()
	if selected then
		codebox:setRaw(SimpleSpy:ValueToVar(selected.ReturnValue, "返回值"))
	end
end)